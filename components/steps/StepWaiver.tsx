...
export function StepWaiver() {
  const { state, setWaiverAccepted, goNext, goPrev } = useBooking();
  const [accepted, setAccepted] = useState(state.waiver_accepted ?? false);

  const additionalNames = state.additional_participants?.map((p) => p.name).filter(Boolean) ?? [];
  const isGroup = additionalNames.length > 0;
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-foreground">Terms &amp; Liability Waiver</h2>
      ...
      <div className="bg-card border border-border rounded-lg p-5 mb-6 max-h-96 overflow-y-auto">
        <h3 className="font-semibold text-foreground mb-3">Liability Waiver &amp; Release of Claims</h3>
        ...
        <div className="prose prose-sm text-muted-foreground space-y-4">
          ...
          <h4 className="font-semibold text-foreground">2. Release of Liability</h4>
          <p>
          ...
          <h4 className="font-semibold text-foreground">3. Medical Acknowledgment</h4>
          <p>
          ...
          <h4 className="font-semibold text-foreground">4. Equipment &amp; Helmet Policy</h4>
          <p>
          ...
          <h4 className="font-semibold text-foreground">5. Cancellation Policy</h4>
          <p>
          ...
          <h4 className="font-semibold text-foreground">6. Photo &amp; Media Release</h4>
          <p>
          ...
          <h4 className="font-semibold text-foreground">7. Governing Law</h4>
          <p>
      ...
      {isGroup && (
        <div className="mb-4 p-4 bg-muted rounded-lg border border-border">
          <p className="text-sm font-semibold text-foreground mb-2">Participants covered by this waiver:</p>
          <ul className="text-sm text-muted-foreground space-y-1">
            <li>• You (lead booker)</li>
      ...
      <div className="flex items-start gap-3 mb-6 p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg">
        <Checkbox
          id="waiver"
          checked={accepted}
        ...
        <Label htmlFor="waiver" className="text-sm text-foreground leading-relaxed cursor-pointer">
          ...
          agree to all terms and conditions{isGroup ? ' on behalf of myself and all participants listed above' : ''}.{' '}
          {isGroup ? 'All participants are' : 'I am'} 18 years of age or older (or have guardian consent for minors).
        </Label>
      </div>
